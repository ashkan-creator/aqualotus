...
import { FaShoppingCart, FaUser, FaChartBar, FaBoxes, FaClipboardList, FaComments, FaUsers, FaLeaf } from 'react-icons/fa'
import { FiMenu, FiSearch, FiSliders, FiFileText, FiSettings, FiList, FiLink, FiLayers } from 'react-icons/fi'
...
const Header = () => {
  ...
  // وضعیت باز و بسته بودن زیرمنوهای ادمین در موبایل
  ...
  return (
    <header className='aq-sticky-header' style={{ zIndex: 1050, position: 'relative' }}>
              ...
              <div className='aq-user-dropdown-container'>
                {userInfo ? (
                  ...
                  >
                    ...
                    {/* اگر ادمین بود منوهای شونصدتایی دسکتاپ را اینجا نشان بده */}
                    {userInfo.isAdmin && (
                      <div className='d-none d-md-block' style={{ minWidth: '220px' }}>
                    ...
                    {/* برای ادمین در موبایل، یک دکمه راهنما می‌گذاریم که برود داخل سایدبار باز کند یا گزینه‌ها را اینجا شیک نمایش دهد */}
                    {userInfo.isAdmin && (
                      <div className='d-md-none text-center bg-light p-2 rounded' style={{ fontSize: '0.8rem', color: '#555', margin: '4px' }}>
                        منوی مدیریت در منوی کشویی ☰ (سمت چپ هدر) ادغام شد.
                      </div>
                    )}
      ...
      {/* سایدبار کشویی (دراور موبایل) کاملاً ادغام‌شده و منظم */}
      <div
        style={{
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ background: '#1b4332', padding: '14px 18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ color: 'white', fontSize: '14px', fontWeight: '500' }}>
                {userInfo?.isAdmin ? '🛠️ پنل مدیریت آکولوتوس' : 'دسته‌بندی محصولات'}
              </span>
            </div>
          ...
          <div style={{ padding: '8px 0' }}>
            {/* اگر ادمین بود، ابتدا منوهای ادمین را در دسته‌بندی‌های شیک آکاردئونی باز کن */}
            {userInfo?.isAdmin && (
              <>
                <SectionLabel>⚡ دسترسی سریع مدیریت</SectionLabel>
                <DrawerItem icon='📊' label='داشبورد اصلی' onClick={() => goTo('/admin/dashboard')} />
                <DrawerItem icon='📈' label='گزارش‌گیری پیشرفته' onClick={() => goTo('/admin/reports')} />
                
                <DrawerSection icon='🛒' label='مدیریت فروش و کالا' color='#2a9d8f' isOpen={adminShopOpen} onToggle={() => setAdminShopOpen(!adminShopOpen)}>
                  <SubItem label='📦 لیست محصولات' onClick={() => goTo('/admin/productlist')} />
                  <SubItem label='💳 مدیریت سفارش‌ها' onClick={() => goTo('/admin/orderlist')} />
                  <SubItem label='💬 نظرات و پاسخ‌ها' onClick={() => goTo('/admin/reviews')} />
                  <SubItem label='👥 لیست کاربران سایت' onClick={() => goTo('/admin/userlist')} />
                  <SubItem label='🌿 خانواده‌های گیاهی' onClick={() => goTo('/admin/familylist')} />
                </DrawerSection>

                <DrawerSection icon='📝' label='مدیریت محتوا و صفحات' color='#e9c46a' isOpen={adminContentOpen} onToggle={() => setAdminContentOpen(!adminContentOpen)}>
                  <SubItem label='🖼️ مدیریت اسلایدرها' onClick={() => goTo('/admin/sliders')} />
                  <SubItem label='📝 پست‌های وبلاگ' onClick={() => goTo('/admin/blog')} />
                  <SubItem label='🔗 لینک‌ساز هوشمند' onClick={() => goTo('/admin/linkpages')} />
                  <SubItem label='🏗️ صفحه‌ساز پیشرفته' onClick={() => goTo('/admin/custompages')} />
                </DrawerSection>

                <DrawerSection icon='⚙️' label='تنظیمات و امنیت' color='#e76f51' isOpen={adminSystemOpen} onToggle={() => setAdminSystemOpen(!adminSystemOpen)}>
                  <SubItem label='⚙️ تنظیمات عمومی سایت' onClick={() => goTo('/admin/settings')} />
                  <SubItem label='📋 لاگ فعالیت سیستم' onClick={() => goTo('/admin/activity-log')} />
                </DrawerSection>
                <Divider />
              </>
            )}
...
const DrawerSection = ({ icon, label, color, isOpen, onToggle, children }) => (
  <>
    ...
    <div style={{ overflow: 'hidden', maxHeight: isOpen ? '300px' : '0', transition: 'max-height 0.3s' }}>
...
print("منوی شونصد تایی ادمین در موبایل کاملاً طبقه‌بندی و مهار شد!")
