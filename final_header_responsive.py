      ...
      <Navbar className='aqualotus-navbar py-1' style={{ direction: 'rtl', minHeight: '65px' }}>
        <Container fluid='md'>
          <div className='d-flex align-items-center justify-content-between w-100' style={{ gap: '8px' }}>
            ...
            {/* بخش راست: لوگو بزرگ و منوهای اصلی */}
            <div className='d-flex align-items-center' style={{ gap: '10px', minWidth: 0, flexShrink: 0 }}>
              <LinkContainer to='/'>
                <Navbar.Brand className='brand-logo d-flex align-items-center m-0'>
                  <img 
                    src='/logo.png' 
                    alt='AquaLotus' 
                    style={{ height: '76px', width: '76px', objectFit: 'contain', marginTop: '-8px', marginBottom: '-8px' }} 
                  />
                </Navbar.Brand>
              </LinkContainer>
            ...
            {/* بخش چپ: گروه کنترل آیکون‌ها و سرچ */}
            <div className='d-flex align-items-center justify-content-end' style={{ gap: '8px', flex: 1, minWidth: 0 }}>
              
              {/* سرچ باکس دسکتاپ */}
              <div className='d-none d-md-flex' style={{ maxWidth: '210px', width: '100%', minWidth: '130px' }}>
                <SearchBox />
              </div>
              ...
              <button
                ...
                style={{ background: 'rgba(255,255,255,0.12)', border: 'none', cursor: 'pointer', borderRadius: '4px', padding: '6px', display: 'flex', alignItems: 'center' }}
              >
          ...
          {/* سرچ ردیف پایین مخصوص موبایل */}
          {mobileSearchOpen && (
            <div className='d-md-none w-100' style={{ padding: '8px 0 4px' }}>
              <SearchBox />
            </div>
          )}
        </Container>
...
const DrawerItem = ({ icon, label, onClick }) => (
  <div
    onClick={onClick}
    style={{ padding: '10px 18px', display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}
  >
...
const SubItem = ({ label, onClick }) => (
  <div
    onClick={onClick}
    style={{ padding: '7px 0', fontSize: '13px', color: '#666', cursor: 'pointer' }}
  >
    {label}
  </div>
)
...
print("فایل Header.jsx با تراز نهایی و لوگوی 76px آپدیت شد!")
