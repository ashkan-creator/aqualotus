    ...
    <header className='aq-sticky-header' style={{ zIndex: 1050, position: 'relative' }}>
      <Navbar className='aqualotus-navbar py-1' style={{ direction: 'rtl', minHeight: '65px' }}>
        <Container fluid='md'>
          <div className='d-flex align-items-center justify-content-between w-100' style={{ gap: '6px' }}>
            ...
            {/* بخش راست: لوگو و منوی اصلی دسکتاپ */}
              ...
              <div className='aq-user-dropdown-container'>
                {userInfo ? (
                  <NavDropdown
                    ...
                    align='end'
                  >
                    {!userInfo.isAdmin && <LinkContainer to='/profile'><NavDropdown.Item>پنل کاربری</NavDropdown.Item></LinkContainer>}
                    {userInfo.isAdmin && (
                      <>
      ...
      <div 
        style={{
          ...
          pointerEvents: mobileSearchOpen ? 'all' : 'none',
          direction: 'rtl'
        }}
      >
        <div style={{ flex: 1 }}>
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ background: '#1b4332', padding: '14px 18px', display: 'flex', alignItems: 'center', justifyBontent: 'space-between', flexShrink: 0, justifyContent: 'space-between' }}>
...
const DrawerSection = ({ icon, label, color, isOpen, onToggle, children }) => (
  <>
    <div onClick={onToggle} style={{ padding: '10px 18px', display: 'flex', alignItems: 'center', gap: '10px', cursor:'pointer' }}>
      <span style={{ fontSize: '16px' }}>{icon}</span>
      <span style={{ fontSize: '14px', color: '#333' }}>{label}</span>
      <span style={{ marginRight: 'auto', color: '#aaa', fontSize: '12px', transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}>▼</span>
    </div>
...
const SubItem = ({ label, onClick }) => <div onClick={onClick} style={{ padding: '7px 0', fontSize: '13px', color: '#666', cursor: 'pointer' }}>{label}</div>
...
print("هدر با سیستم موشن انیمیشن سرچ تمام‌صفحه با موفقیت بروزرسانی شد!")
