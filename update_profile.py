...
code_content = """import { useState, useEffect } from 'react'
...
const ComingSoon = ({ title, icon }) => (
  <Card className='auth-card'>
    <Card.Body className='p-5 text-center text-muted'>
      <i className={`fa-solid ${icon} mb-3`} style={{ fontSize: '2.5rem', opacity: 0.4 }}></i>
      <h5 className='mb-2'>{title}</h5>
      <p className='mb-0' style={{ fontSize: '14px' }}>این بخش به‌زودی تکمیل می‌شود</p>
    </Card.Body>
  </Card>
)
  ...
  return (
    <Card className='auth-card'>
      <Card.Body className='p-4'>
        ...
        {userInfo?.isAdmin && <Badge bg='danger' className='mb-3 px-3 py-2' style={{ fontSize: '13px' }}>⚡ دسترسی مدیریت سیستم</Badge>}
        <p className='text-muted' style={{ fontSize: '14px' }}>
          خلاصه‌ای از وضعیت حساب کاربری شما
        </p>
      ...
      {loadingOrders ? (
        ...
        <>
          <div className='d-md-none'>
            {orders.map((order) => (
              <Card key={order._id} className='mb-3 order-mobile-card'>
                <Card.Body>
                  ...
                  <div className='text-muted mb-2' style={{ fontSize: '13px', lineHeight: '1.8' }}>
                    {order.orderItems?.map((i) => `${i.name}${i.selectedSize ? ` (${i.selectedSize})` : ''}`).join('، ')}
                  </div>
                  <div className='d-flex justify-content-between align-items-center mb-2'>
                    <span className='text-muted' style={{ fontSize: '13px' }}>مبلغ:</span>
                    <strong>{Math.round(order.totalPrice).toLocaleString('fa-IR')} تومان</strong>
                  </div>
                  ...
                  <Link to={`/order/${order._id}`}>
                    <Button size='sm' className='w-100 btn-aqualotus'>
                      جزئیات
                    </Button>
                  </Link>
                </Card.Body>
...
const ProfilePage = () => {
  ...
  const adminTabs = [
    { key: 'dashboard', label: 'داشبورد ادمین', icon: 'fa-gauge-high' },
    { key: 'admin-orders', label: '📦 سفارشات کل سایت', icon: 'fa-boxes-packing', isLink: true, path: '/admin/orderlist' },
    { key: 'admin-products', label: '🌱 مدیریت محصولات', icon: 'fa-seedling', isLink: true, path: '/admin/productlist' },
    { key: 'admin-users', label: '👥 مدیریت کاربران', icon: 'fa-users', isLink: true, path: '/admin/userlist' },
    { key: 'profile', label: 'تنظیمات حساب', icon: 'fa-user-gear' },
  ]
  ...
  const currentTabs = userInfo?.isAdmin ? adminTabs : userTabs

  const handleTabSelect = (key) => {
    const selectedTab = currentTabs.find(tab => tab.key === key)
  ...
  return (
    <>
      ...
      <Container className='py-4'>
        <h4 className='mb-4'>{userInfo?.isAdmin ? 'پنل مدیریت آکوالوتوس' : 'پنل کاربری'}</h4>
        <Row>
          <Col md={3} className='mb-3 mb-md-0'>
            <Card className='auth-card'>
              <Card.Body className='p-2'>
                <Nav
                  variant='pills'
                  className='customer-panel-nav flex-column'
                  activeKey={activeTab}
                  onSelect={handleTabSelect}
                >
                  {currentTabs.map((tab) => (
                    <Nav.Item key={tab.key}>
                      <Nav.Link eventKey={tab.key} className={tab.isLink ? 'admin-redirect-link' : ''}>
          ...
          <Col md={9}>
            {activeTab === 'dashboard' && (
              <DashboardTab userInfo={userInfo} orders={orders} loadingOrders={loadingOrders} />
            )}
            {activeTab === 'orders' && (
              <OrdersTab orders={orders} loadingOrders={loadingOrders} error={error} />
            )}
...
target_path = 'src/pages/ProfilePage.jsx'

# مطمئن می‌شویم مسیر پوشه وجود دارد
os.makedirs(os.path.dirname(target_path), exist_ok=True)

with open(target_path, 'w', encoding='utf-8') as f:
    f.write(code_content)

print("✅ فایل ProfilePage.jsx با موفقیت بروزرسانی شد.")
